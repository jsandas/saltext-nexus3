#!/usr/bin/env python3
"""Generate docs markdown from function docstrings in src modules and states.

This script scans top-level Python files under src/saltext/nexus3/modules and
src/saltext/nexus3/states, extracts top-level function signatures and
docstrings, then writes markdown files under:
...
def main() -> int:
    ...
    updates.extend(write_docs(root / "src" / "saltext" / "nexus3" / "modules", root / "docs" / "modules", check_only=args.check))
    updates.extend(write_docs(root / "src" / "saltext" / "nexus3" / "states", root / "docs" / "states", check_only=args.check))
