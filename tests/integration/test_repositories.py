...
def test_group():
    ret = client.cmd(
        "test.minion",
        "nexus3_repositories.group",
        ["name=test-yum-group", "format=yum", 'group_members=["test-yum-hosted"]'],
    )
    # print(ret)
    data = ret["test.minion"]["repository"]
...
def test_proxy():
    ret = client.cmd(
        "test.minion",
        "nexus3_repositories.proxy",
        ["name=test-yum-proxy", "format=yum", "remote_url=https://randomurl.com"],
    )
    # print(ret)
    data = ret["test.minion"]["repository"]
    ...
    ret = client.cmd(
        "test.minion",
        "nexus3_repositories.proxy",
        ["name=test-apt-proxy", "format=apt", "remote_url=https://randomurl.com"],
    )
    # print(ret)
    data = ret["test.minion"]["repository"]
...
def test_proxy_with_auth():
    ret = client.cmd(
        ...
        "nexus3_repositories.proxy",
        [
            "name=test-yum-proxy_auth",
            "format=yum",
            "remote_url=https://randomurlauth.com",
    ...
    assert (
        data["httpClient"]["authentication"]["type"] == "username"
    ), "authentication type is incorrect"
    assert (
        data["httpClient"]["authentication"]["username"] == "test_user"
    ), "authentication username is incorrect"
...
def test_proxy_docker_subdomain():
    ...
    ret = client.cmd(
        ...
        "nexus3_repositories.proxy",
        [
            "name=test-docker-subdomain-module",
            "format=docker",
            "remote_url=https://registry-1.docker.io",
            "docker_http_port=5011",
            "docker_index_type=HUB",
...
def test_proxy_docker_switch_path_to_subdomain():
    ...
    ret = client.cmd(
        ...
        [
            ...
            "remote_url=https://registry-1.docker.io",
            "docker_index_type=HUB",
            "docker_subdomain=registry-path-switch",
        ],
    )
    # print(ret)
    data = ret["test.minion"]["repository"]
...
def test_proxy_docker_switch_subdomain_to_path():
    ...
    assert (
        data["docker"]["subdomain"] is None
    ), "subdomain must be None after switching to path routing"
...
def test_describe():
    ...
    assert (
        ret["test.minion"]["repository"]["name"] == "test-yum-hosted"
    ), "repository test-yum-hosted not found"
    ...
    assert ret["test.minion"]["repository"] != {}, "data is empty"
    assert (
        ret["test.minion"]["repository"]["name"] == "test-apt-proxy"
    ...
    "results": {
        ...
        "apt-proxy": {
            ...
            "changes": {
                ...
                "cleanup": None,
                "apt": {"distribution": "bionic", "flat": True, "enforceDistribution": False},
                "proxy": {
        ...
        "apt-proxy-http-client": {
            ...
            "changes": {
                ...
                "cleanup": None,
                "apt": {"distribution": "bionic", "flat": False, "enforceDistribution": False},
                "proxy": {
                    "remoteUrl": "http://apt-proxy",
                    "contentMaxAge": 1440,
                    "metadataMaxAge": 1440,
...
def test_apt_repository_state():
    ...
    for key, values in state_test_apt_data["results"].items():
        ...
        assert (
            values["changes"] == output["changes"]
        ), f"wrong changes result! expected: \"{values['changes']}\" got: \"{output['changes']}\""
...
def test_docker_repository_state():
    ...
    for key, values in state_test_docker_data["results"].items():
        ...
        assert (
            values["changes"] == output["changes"]
        ), f"wrong type result! expected: \"{values['changes']}\" got: \"{output['changes']}\""
...
def test_docker_path_routing_state():
    ...
    for key, values in state_test_docker_path_data["results"].items():
        ...
        assert (
            values["changes"] == output["changes"]
        ), f"wrong changes! expected: \"{values['changes']}\" got: \"{output['changes']}\""
