...
def test_create_privileges():
    ...
    assert ret["test.minion"]["privilege"]["actions"] == [
        "ADD",
        "READ",
    ], "privilege action is incorrect"
    assert (
        ret["test.minion"]["privilege"]["domain"] == "userschangepw"
    ), "privilege domain is incorrect"
    assert (
        ret["test.minion"]["privilege"]["type"] == "application"
    ), "privilege privilege is incorrect"
...
def test_update_privileges():
    ret = client.cmd(
        "test.minion",
        "nexus3_privileges.update",
        ["name=testing1", "description='New Description'", "domain=users"],
    )
    # print(ret)
    assert ret["test.minion"]["privilege"] != {}, "privilege is empty"
    assert (
        ret["test.minion"]["privilege"]["description"] == "New Description"
    ), "privilege description is incorrect"
    assert ret["test.minion"]["privilege"]["domain"] == "users", "privilege domain is incorrect"

    ret = client.cmd(
        "test.minion",
        "nexus3_privileges.update",
        ["name=testing3", "description='New Description'", "domain=users"],
    )
    # print(ret)
