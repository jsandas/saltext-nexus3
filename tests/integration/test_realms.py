...
def test_update_realms():
    ret = client.cmd(
        "test.minion", "nexus3_realms.update", [["NexusAuthenticatingRealm", "DockerToken"]]
    )
    # print(ret)
    assert ret["test.minion"]["realms"] != {}, "realms is empty"
    assert ret["test.minion"]["realms"] == [
...
def test_active_realms():
    client.cmd(
        "test.minion",
        "nexus3_realms.update",
        [["NexusAuthenticatingRealm", "DockerToken", "NpmToken"]],
    )
